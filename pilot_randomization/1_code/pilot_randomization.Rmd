...
the treatment allocation code for the pilot. This includes:
  ...
  * verifying that each participant has at least Tuesday availability or Thursday availability.
...
allocation code. The data should be in a dataframe with the following columns:
  ...
  * avail_tuesday: a character variable taking values "yes" or "no" indicating whether the participant is available on Tuesday
  * avail_thursday: a character variable taking values "yes" or "no" indicating whether the participant is available on Thursday
  ...
  * trt: a character variable indicating the treatment assignment for the participant. The expected values of this variable are:
      * "tuesday" for those already assigned to the treatment administered on Tuesdays
      * "thursday" for those already assigned to the treatment administered on Thursdays
...
names(enrolled_data) <- c("record_id", "num", "avail_tuesday", "avail_thursday", 
                          "age", "gender", "race", "trt", "seed") # num is rank order in which the person was enrolled
enrolled_data[1,] <- c(1, 3, "yes", "yes", "50-59", "F", "white", "tuesday", "1234")
...
There is one additional function available that you can use to map between calling the treatment "tuesday" and "thursday" and  the actual treatment names. 

```{r, echo = TRUE}
convertFromDayToTrtName(enrolled_data_updated, tuesday_is_ReCharge = TRUE)
```

This may be useful or not. Just remember that all of the code is using the "tuesday" and "thursday" convention for the treatment names. This is so you can look at the day availability and the treatment assignment and not need to do mental gymnastics to figure out if the treatment assignment makes sense. On the other hand, it's not exactly what we'll want to put into REDCap. 
...
enrolled_data_deidentified %>%
  add_column(avail = sample(x = c("tuesday", "thursday", "both"), size = nrow(enrolled_data_deidentified), replace = TRUE), .after = "race") %>%
  mutate(avail_tuesday = case_when(avail == "tuesday" | avail == "both" ~ "yes",
                                   TRUE ~ "no"),
         avail_thursday = case_when(avail == "thursday" | avail == "both" ~ "yes",
                                    TRUE ~ "no")) %>%
